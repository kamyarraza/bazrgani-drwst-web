export const endPoints = {
  ...
  notification: {
    ...
    markAsRead: (notificationId: string | number) => `/notifications/${notificationId}/mark-as-read`,
...
// location endpoint removed as it's duplicated by the more complete locations endpoint
  ...
  branch: {
    ...
    toggleActive: (branchId: string | number) => `/branches/toggle-active/${branchId}`
  },
  warehouse: {
    ...
    toggleActive: (warehouseId: string | number) => `/warehouses/toggle-active/${warehouseId}`
  },

  // Add missing endpoint for branch warehouses - supports search with ?query=something
  branchWarehouses: (branchId: string | number) => `/warehouses/${branchId}/index`,
  ...
  warehouseItems: {
    ...
    update: (warehouseId: string | number, itemId: string | number) => `/warehouses/${warehouseId}/items/${itemId}`,
    delete: (warehouseId: string | number, itemId: string | number) => `/warehouses/${warehouseId}/items/${itemId}`
  },
  ...
  itemCategory: {
    ...
    details: (itemCategoryId: string | number) => `/item-categories/${itemCategoryId}`,
    update: (itemCategoryId: string | number) => `/item-categories/${itemCategoryId}`,
    toggleStatus: (itemCategoryId: string | number) => `/item-categories/toggle-status/${itemCategoryId}`
  },

  transactions: {
    list: '/transactions',
    create: '/transactions',
    details: (transactionId: string | number) => `/transactions/${transactionId}`,
    update: (transactionId: string | number) => `/transactions/${transactionId}`,
    delete: (transactionId: string | number) => `/transactions/${transactionId}`
  },
  report: {
  ...
  offer: {
    ...
    changeStatus: (offerId: string | number, status: string) => `/offers/${offerId}/${status}`
  }
  ...
  warehouseItemTransfer: {
    ...
    approve: (transferId: string | number) => `/warehouse/items/transfer/${transferId}/approve`,
    // Marks an approved warehouse item transfer as completed and adds the items to the destination warehouse.
    complete: (transferId: string | number) => `/warehouse/items/transfer/${transferId}/complete`,
    // Rejects a pending warehouse item transfer request.
    reject: (transferId: string | number) => `/warehouse/items/transfer/${transferId}/reject`
  },
