<template>
  <q-dialog v-model="isOpen" persistent transition-show="scale" transition-hide="scale" class="cute-modal" maximized
    :full-width="false" :full-height="false" v-if="props.set">
    <q-card class="cute-updateset-card scrollable-card">
      <!-- Header with gradient background -->
      <q-card-section class="cute-header">
        <div class="header-content">
          <div class="header-icon">
            <q-icon name="edit" size="2rem" class="header-icon-inner" />
          </div>
          <div class="header-text">
            <div class="title">{{ t('blum.updateSet') }}</div>
            <div class="subtitle">{{ t('blum.updateSetSubtitle', 'Modify your Blum component set') }}</div>
          </div>
        </div>
          ...
          <q-form @submit.prevent="onSubmit" class="cute-form">
            ...
            <div class="form-section-wrapper">
              ...
              <div class="row q-gutter-sm">
                ...
                <div class="col-12 col-md-6">
                  <div v-if="imagePreview || currentSetImage" class="image-preview-container">
                    <div class="preview-label">
                      {{ imagePreview ? t('blum.newImagePreview', 'New Image Preview') : t('blum.currentImage', 'Current Image') }}
                    </div>
                    <div class="preview-wrapper">
                      <q-img :src="imagePreview || currentSetImage || ''" class="preview-image" fit="cover"
                        loading="lazy">
                        <template v-slot:loading>
                          <q-spinner-gears color="primary" size="2em" />
                        </template>
                      </q-img>
                      <q-btn v-if="imagePreview" round dense flat icon="close" color="negative" size="sm"
                        class="remove-image-btn" @click="removeImage">
            ...
            <div class="form-section-wrapper">
              ...
              <div class="row q-gutter-sm">
                ...
                <div class="col-12 col-md-4">
                  <q-btn color="primary" icon="search" :label="t('common.search')" @click="onSearchItems"
                    ...
                    <q-tooltip v-if="!blumStore.blumItemsLoading">{{ t('blum.searchForItems', 'Search for items to add to your set') }}</q-tooltip>
                  </q-btn>
                </div>
              </div>
              ...
              <div v-if="searchResults.length > 0" class="search-results-section">
                <div class="results-header">
                  ...
                  <q-chip :label="t('blum.clickToAdd', 'Click to add')" color="primary" text-color="white" size="sm"
                    icon="add_circle" />
                  <q-space />
            ...
            <div v-if="form.items.length > 0" class="form-section-wrapper">
              <div class="section-header">
                ...
                <span class="section-title">{{ t('blum.currentItems', 'Current Items') }} ({{ form.items.length
                  }})</span>
                <q-space />
            ...
            <div class="action-section">
              ...
              <q-btn type="submit" :label="t('common.save')" color="primary" :loading="blumStore.blumSetsLoading"
...
function getItemDisplayName(itemId: number): string {
  ...
  // Fallback: look in the store items
  const storeItem = blumStore.blumItems.find(item => item.id === itemId);
  return storeItem ? storeItem.name : `Item ID: ${itemId}`;
}
</script>
