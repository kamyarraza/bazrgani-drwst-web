      ...
      <div class="row items-center q-mt-md">
        <div class="col-auto">
          <div class="filter-buttons">
            <q-btn
              :label="t('warehouseItem.regularItems', 'Regular Items')"
              :color="selectedRelationType === 'items' ? 'primary' : 'grey-5'"
              :flat="selectedRelationType !== 'items'"
              :icon="selectedRelationType === 'items' ? 'inventory' : 'inventory'"
              no-caps
              class="q-mr-sm filter-btn"
              @click="handleRelationTypeChange('items')"
            />
            <q-btn
              :label="t('warehouseItem.blumItems', 'Blum Items')"
              :color="selectedRelationType === 'blum_items' ? 'primary' : 'grey-5'"
              :flat="selectedRelationType !== 'blum_items'"
              :icon="selectedRelationType === 'blum_items' ? 'inventory_2' : 'inventory_2'"
              no-caps
              class="filter-btn"
    ...
    <div class="warehouse-items-content">
        ...
        <template #body-cell-unit_cost="props">
          <q-td :props="props" class="text-right">
            <q-badge
              color="positive"
              text-color="white"
              :label="`$${parseFloat(props.value || '0').toFixed(2)}`"
              class="price-badge"
            />
          </q-td>
        </template>
      ...
      <q-card v-else class="no-items-card text-center q-pa-xl">
        ...
        <q-btn
          ...
          @click="props.selectedWarehouse && loadWarehouseItems(props.selectedWarehouse.id)"
          class="q-mt-md"
        />
      </q-card>
    </div>
  </div>    <div class="warehouse-select-prompt text-center q-pa-xl" v-else>
    ...
    <q-card style="min-width: 600px; max-width: 900px;">
      ...
      <template v-if="selectedItemForView">
        <q-card-section class="q-pt-md">
          ...
          <div class="row q-col-gutter-md">
            <div class="col-12 col-md-8">
              ...
              <div class="text-subtitle2 text-grey-7">
                ...
                <q-chip
                  v-if="'code' in selectedItemForView && selectedItemForView.code"
                  color="blue-grey"
                  text-color="white"
                  size="sm"
                  class="q-ml-sm"
                ...
                <q-chip
                  v-if="'part_no' in selectedItemForView && selectedItemForView.part_no"
                  color="indigo"
                  text-color="white"
                  size="sm"
                  class="q-ml-sm"
          ...
          <div class="row q-col-gutter-md">
            ...
            <div class="col-12 col-md-6">
              ...
              <q-list dense bordered class="rounded-borders">
                <q-item>
                  <q-item-section>
                    <q-item-label class="text-weight-medium">{{ t('warehouseItem.unitCost', 'Unit Cost') }}</q-item-label>
                    <q-item-label caption class="text-h6 text-positive">${{ parseFloat(selectedItemForView.unit_cost || '0').toFixed(2) }}</q-item-label>
                  </q-item-section>
                </q-item>
                <q-separator />
                <q-item v-if="'solo_unit_price' in selectedItemForView">
                  <q-item-section>
                    <q-item-label class="text-weight-medium">{{ t('warehouseItem.soloPrice', 'Solo Unit Price') }}</q-item-label>
                    <q-item-label caption class="text-h6 text-primary">${{ parseFloat(selectedItemForView.solo_unit_price || '0').toFixed(2) }}</q-item-label>
                  </q-item-section>
                </q-item>
                <q-separator v-if="'solo_unit_price' in selectedItemForView" />
                <q-item v-if="'bulk_unit_price' in selectedItemForView">
                  <q-item-section>
                    <q-item-label class="text-weight-medium">{{ t('warehouseItem.bulkPrice', 'Bulk Unit Price') }}</q-item-label>
                    <q-item-label caption class="text-h6 text-orange">${{ parseFloat(selectedItemForView.bulk_unit_price || '0').toFixed(2) }}</q-item-label>
                  </q-item-section>
                </q-item>
                <q-item v-if="'unit_price' in selectedItemForView">
                  <q-item-section>
                    <q-item-label class="text-weight-medium">{{ t('warehouseItem.unitPrice', 'Unit Price') }}</q-item-label>
                    <q-item-label caption class="text-h6 text-primary">${{ parseFloat(selectedItemForView.unit_price || '0').toFixed(2) }}</q-item-label>
                  </q-item-section>
                </q-item>
              </q-list>
            </div>
            ...
            <div class="col-12 col-md-6">
              ...
              <q-list dense bordered class="rounded-borders">
                <q-item>
                  <q-item-section>
                    <q-item-label class="text-weight-medium">{{ t('warehouseItem.quantity', 'Total Quantity') }}</q-item-label>
                    <q-item-label caption class="text-subtitle1 text-weight-medium">
                      <q-badge
                        ...
                        :label="String(selectedItemForView.quantity || 0)"
                        size="sm"
                        class="quantity-badge-small"
                      />
                    </q-item-label>
                  </q-item-section>
                </q-item>
                ...
                <q-item v-if="'pieces' in selectedItemForView">
                  <q-item-section>
                    ...
                    <q-item-label caption class="text-subtitle1 text-weight-medium">{{ selectedItemForView.pieces || 0 }}</q-item-label>
                  </q-item-section>
                </q-item>
                <q-separator v-if="'reservations' in selectedItemForView" />
                <q-item v-if="'reservations' in selectedItemForView">
                  <q-item-section>
                    <q-item-label class="text-weight-medium">{{ t('warehouseItem.reservations', 'Reserved Items') }}</q-item-label>
                    <q-item-label caption class="text-subtitle1 text-weight-medium">
                      <q-badge
                        color="info"
                        :label="String(selectedItemForView.reservations || 0)"
                        size="sm"
                ...
                <q-item v-if="'position' in selectedItemForView && selectedItemForView.position">
                  <q-item-section>
                    <q-item-label class="text-weight-medium">{{ t('warehouseItem.position', 'Position') }}</q-item-label>
                    <q-item-label caption class="text-subtitle1 text-weight-medium">{{ selectedItemForView.position }}</q-item-label>
                  </q-item-section>
                </q-item>
              </q-list>
            </div>
          ...
          <div class="q-mt-md" v-if="'packets' in selectedItemForView && (selectedItemForView.packets || selectedItemForView.packages)">
            <div class="text-h6 q-mb-sm text-primary">
            ...
            <q-card flat bordered class="bg-grey-1">
              <q-card-section>
                <div class="row items-center">
                  <div class="col">
                    <div class="text-subtitle1 text-weight-medium">{{ t('warehouseItem.availability', 'Availability') }}</div>
                  </div>
                </div>
                <div class="text-caption text-grey-6 q-mt-sm">
                  {{ t('warehouseItem.availableForSale', 'Available for sale') }}:
                  <strong>{{ (selectedItemForView.quantity || 0) - ('reservations' in selectedItemForView ? selectedItemForView.reservations || 0 : 0) }}</strong> {{ t('warehouseItem.units', 'units') }}
                </div>
              </q-card-section>
            </q-card>
          </div>
