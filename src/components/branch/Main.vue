...
const columns = [
  ...
  },
  {
    ...
    align: 'left' as const,
    field: (row: any) => (parseFloat(row.purchase_borrow) > 0 && (isAdmin.value || row.id === userBranchId.value)) ? formatCurrency(row.purchase_borrow) : '🌱',
    sortable: true,
    style: (val: any) => ({
  ...
  },
  {
    ...
    field: (row: any) => (parseFloat(row.sell_borrow) > 0 && (isAdmin.value || row.id === userBranchId.value)) ? formatCurrency(row.sell_borrow) : '🌷',
    sortable: true,
    style: (val: any) => ({
