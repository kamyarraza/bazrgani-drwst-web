<template>
  <q-dialog v-model="show" persistent transition-show="slide-up" transition-hide="slide-down">
    <q-card class="add-category-card">
      <!-- Header with gradient background -->
      <q-card-section class="card-header">
        <div class="header-content">
          <div class="header-icon-wrapper">
            <q-icon name="add_circle" class="header-icon" />
          </div>
          <div class="header-text">
            <div class="text-h6">{{ t('expenseCategory.addNew') }}</div>
            <div class="text-caption text-grey-6">{{ t('expenseCategory.addSubtitle', 'Create a new expense category')
              }}</div>
          </div>
        </div>
      ...
      <q-card-section class="form-content">
        <q-form @submit="onSubmit" @reset="onReset" class="form-container">
          ...
          <div class="button-group">
            <q-btn :label="t('expenseCategory.addCategory', 'Add Category')" type="submit" color="primary"
              :loading="expenseCategoryStore.loading" class="action-btn primary-btn" icon="add_circle_outline" no-caps>
      ...
      <div v-if="showSuccess" class="success-overlay">
        <div class="success-content">
          ...
          <div class="success-text">{{ t('expenseCategory.categoryAdded', 'Category Added Successfully!') }}</div>
        </div>
      </div>
    </q-card>
  </q-dialog>
...
async function onSubmit() {
  try {
    await expenseCategoryStore.createCategory(form.value);
