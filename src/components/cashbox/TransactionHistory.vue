    ...
    <div class="transactions-content">
      <template v-if="transactions && transactions.length > 0">
        <div class="transactions-list">
          <div v-for="transaction in reversedTransactions" :key="transaction.id" class="transaction-item" :class="{
              ...
              <div class="transaction-main">
                ...
                <div class="transaction-amounts" v-if="hasAmounts(transaction)">
                  <div class="amount-item" v-if="transaction.total_iqd">
                    <q-chip :color="transaction.total_iqd > 0 ? 'amber-6' : 'red-6'" text-color="white" size="sm"
                  ...
                  <div class="amount-item" v-if="transaction.total_usd">
                    <q-chip :color="transaction.total_usd > 0 ? 'green-6' : 'red-6'" text-color="white" size="sm"
            ...
            <div class="transaction-badge">
              <div class="badge-container">
                <q-chip :color="isPositiveTransaction(transaction) ? 'green-6' : 'red-6'" text-color="white" size="md"
                  :icon="isPositiveTransaction(transaction) ? 'trending_up' : 'trending_down'"
...
function isPositiveTransaction(transaction: CashboxTransaction) {
  // Check if the total amounts indicate a positive (incoming) transaction
  const totalIqd = transaction.total_iqd || 0;
  const totalUsd = transaction.total_usd || 0;

  // If both amounts exist, consider positive if either is positive
  // If only one exists, use that one's sign
  if (totalIqd !== 0 && totalUsd !== 0) {
    return totalIqd > 0 || totalUsd > 0;
  } else if (totalIqd !== 0) {
    return totalIqd > 0;
  } else if (totalUsd !== 0) {
    return totalUsd > 0;
  ...
  // If no amounts, fallback to tag-based logic
  return transaction.tag === 'deposit';
}
</script>
